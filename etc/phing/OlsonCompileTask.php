...
class OlsonCompileTask extends Task
{
	...
	public function main()
	{
		...
		require_once('src/quiote.php');

		$this->olsonDir = realpath($this->olsonDir);
		$this->outputDir = realpath($this->outputDir);

		QuioteConfig::set('olson.dir', $this->olsonDir);

		QuioteConfig::set('core.app_dir', getcwd() . '/etc/olson/quiote/app');
		Quiote::bootstrap('');

		$context = QuioteContext::getInstance('');
		...
		foreach($di as $file) {
			if($file->isFile()) {
				...
				if(!str_contains($file->getFilename(), '.') && $file->getFilename() != 'factory') {
					...
					$parser = new QuioteTimeZoneDataParser();
					$parser->initialize(QuioteContext::getInstance($context));
