...
if(!isset($_POST['step'])) {
	...
	$_POST['mode'] = 'rewrite';
	$_POST['prefix'] = dirname((string) $_SERVER['PHP_SELF']);
	if($_POST['prefix'] == '/') {
		$_POST['prefix'] = '';
	}
} elseif($_POST['step'] == 0) {
...
$_POST['base'] = dirname((string) $_SERVER['PHP_SELF']);
if($_POST['base'] != '/') {
	$_POST['base'] .= '/';
}
...
<input type="hidden" name="prefix" value="<?php echo htmlspecialchars((string) $_POST['prefix']); ?>" />
<input type="hidden" name="step" value="<?php echo $_POST['step']+1; ?>" />
<?php
foreach($_POST['data'] as $step => $data):
?>
<input type="hidden" name="data[<?php echo $step; ?>]" value="<?php echo htmlspecialchars((string) $data); ?>" />
<?php
endforeach;
?>
<input type="hidden" name="data[<?php echo $_POST['step']; ?>]" value="<?php echo htmlspecialchars(serialize(['_ENV' => $_ENV, '_GET' => $_GET, '_SERVER' => $_SERVER])) ; ?>" />
<?php
if($_POST['step'] > -1):
?>
