...
abstract class Routing
{
	...
	// Application context (Quiote framework) – exposed to subclasses as $this->context for legacy compatibility
	...
	public function addRoute(string $pattern, array $opts = [], ?string $parent = null): string
	{
		...
		if (isset($this->meta[$name])) {
			...
			if ($existingParent !== $parent) {
				throw new QuioteException("Route name '$name' already exists with different parent");
			}
		}
	...
	private function buildAuthority(string $host, ?int $port, string $scheme, bool $forcePort): string
	{
		...
		if ($forcePort || Toolkit::isPortNecessary($scheme, $port)) {
