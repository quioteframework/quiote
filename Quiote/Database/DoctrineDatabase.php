...
class DoctrineDatabase extends Database
{
    ...
    public function initialize(DatabaseManager $databaseManager, array $parameters = [])
	{
		...
		if(!$is12) {
		  trigger_error('Support for Doctrine versions older than 1.2 is deprecated and will be removed in Quiote 1.2.', E_USER_DEPRECATED);
		}
		...
		try {
			...
			$cel = $this->getParameter('connection_event_listener_class', 'DoctrineDatabaseEventListener');
			$this->connection->setListener(new $cel($this));
			
			// set the context instance as a connection parameter
			$this->connection->setParam('context', $databaseManager->getContext(), 'org.quiote');
