 ...
 * referencing an Doctrine2dbalDatabase connection.
 ...
 *                  the name (string) of a configured Doctrine2dbalDatabase
...
class Doctrine2ormDatabase extends Doctrine2Database
{
	...
	public function connect()
	{
		...
		if(is_string($connection)) {
			try {
				...
				throw new DatabaseException(sprintf('Doctrine2dbalDatabase connection "%s" configured for use in Doctrine2ormDatabase "%s" could not be found.', $connection, $this->getName()), 0, $e);
			}
			try {
				$connection = $connection->getConnection();
			} catch(DatabaseException $e) {
				throw new DatabaseException(sprintf("Doctrine2dbalDatabase connection '%s' configured for use in Doctrine2ormDatabase '%s' could not be initialized:\n\n%s", $this->getParameter('connection'), $this->getName(), $e->getMessage()), 0, $e);
			}
		} elseif(!is_array($connection)) {
			throw new DatabaseException('Doctrine2ormDatabase expects configuration parameter "connection" to be an array containing connection details or a string with the name of an Doctrine2dbalDatabase to use.');
		}
	...
	protected function prepareConfiguration(\Doctrine\DBAL\Configuration $config)
	{
		...
		$config->setProxyNamespace($this->getParameter('configuration[proxy_namespace]', 'Doctrine2ormDatabase_Proxy_' . preg_replace('#\W#', '_', $this->getName())));
