...
class PsrPipelineBuilder
{
    ...
    public function buildRequestFromGlobals(): ServerRequestInterface
    {
        ...
        /** @var \Quiote\Request\Request $legacyReqTyped */
        $legacyReqTyped = $legacyReq; // help static analysis
        // Construct WebRequest directly (it extends Nyholm\Psr7\ServerRequest)
        $quiote = new WebRequest(
    ...
    public function buildDispatcher(RequestHandlerInterface $finalHandler): RequestHandlerInterface
    {
        ...
        $loggerFn = function(\Throwable $e, ServerRequestInterface $r) use ($context): void {
            ...
            $pieces = [
                '[Pipeline]', $e::class, $e->getMessage(), '@', $first,
                'uri='.$uri,
            ...
            \Quiote\Logging\Log::for($this)->debug($msg);
        };
