...
class ValidationMiddleware implements MiddlewareInterface
{
     ...
     *
     * Requires an ActionDescriptor; without one the request passes through
     * untouched. An ExecutionState is guaranteed on the request. A simple
     * action takes a shortened path that skips validator registration. A
     * decision already recorded on the ExecutionState means validation ran
     * earlier in this request and is not repeated — SecurityMiddleware resets
     * it to pending when it forwards, so a forwarded action is revalidated.
     *
     ...
     *
     * On failure the decision carries the errors and the action's
     * `handle*Error()` view is rendered here rather than by DispatchMiddleware.
     */
