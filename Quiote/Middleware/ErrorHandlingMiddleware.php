...
class ErrorHandlingMiddleware implements MiddlewareInterface
{
    ...
    public function renderExceptionResponse(ServerRequestInterface $request, Throwable $e): ResponseInterface
    {
        ...
        \Quiote\Logging\Log::for($this)->debug(sprintf('[ErrorHandlingMiddleware] content negotiation: accept=%s output_type=%s wants_json=%s', $accept, $outputType, $wantsJson ? '1' : '0'));

        $env = Config::get('core.environment');
        ...
        if ($wantsJson) {
            ...
            if ($jsonTemplate) {
                if (\Quiote\Logging\Log::for($this)->isEnabled(\Quiote\Logging\Level::Debug)) {
                    \Quiote\Logging\Log::for($this)->debug(sprintf('[ErrorHandlingMiddleware] JSON template selected, template=%s mode=%s', $jsonTemplate, $mode));
                }
                return $this->includeTemplateToResponse($jsonTemplate, $status, $request, $e, 'application/json');
            }
            if (\Quiote\Logging\Log::for($this)->isEnabled(\Quiote\Logging\Level::Debug)) {
                \Quiote\Logging\Log::for($this)->debug(sprintf('[ErrorHandlingMiddleware] JSON template missing, falling back to minimal payload'));
            }
            // fallback legacy minimal payload
            $payload = ['error' => $status >= 500 ? 'Internal Server Error' : 'Request Error', 'type' => $e::class];
            if (getenv('QUIOTE_DEBUG')) {
            ...
            return new Response($status, ['Content-Type' => 'application/json; charset=utf-8', 'X-Quiote-Error-Type' => $e::class], $bodyJson);
        }

        // Fast-path (moved after correlation id extraction): dev/plaintext minimal body including message (no template)
        if(!$isProd && getenv('QUIOTE_DEBUG') && str_contains($accept, 'text/plain')) {
            $plain = $e->getMessage();
            if($plain === '') { $plain = $status >= 500 ? 'Internal Server Error' : 'Request Error'; }
            if($cid) { $plain .= "\nCorrelation-Id: " . $cid; }
            return new Response($status, ['Content-Type' => 'text/plain; charset=utf-8', 'X-Quiote-Error-Type' => $e::class], $plain);
        }

        // HTML template resolution extended with new keys
        $htmlStructured = $this->resolveStructuredTemplate('html', $mode);
        if ($htmlStructured && \Quiote\Logging\Log::for($this)->isEnabled(\Quiote\Logging\Level::Debug)) {
            \Quiote\Logging\Log::for($this)->debug(sprintf('[ErrorHandlingMiddleware] HTML template selected, template=%s mode=%s', $htmlStructured, $mode));
        } else {
            \Quiote\Logging\Log::for($this)->debug(sprintf('[ErrorHandlingMiddleware] HTML template missing, attempting legacy'));
        }
        ...
        if ($tplFile && preg_match('/json_(development|production)\.php$/', $tplFile)) {
            \Quiote\Logging\Log::for($this)->debug(sprintf('[ErrorHandlingMiddleware] HTML guard: JSON misconfiguration detected, forcing fallback to HTML'));
        ...
        if ($tplFile && $htmlStructured !== $tplFile) {
            \Quiote\Logging\Log::for($this)->debug(sprintf('[ErrorHandlingMiddleware] HTML legacy template selected, template=%s', $tplFile));
        }
        ...
        try {
            ...
            \Quiote\Logging\Log::for($this)->debug(sprintf('[ErrorHandlingMiddleware] including template %s', $tplFile));
            include $tplFile;
            $body = ob_get_clean();
            ...
            $msg = Config::get('core.use_verbose_exception_template', false) ? 'Template render failed: ' . $renderErr->getMessage() : 'Internal Server Error';
            \Quiote\Logging\Log::for($this)->error(sprintf('[ErrorHandlingMiddleware] Template render failed: %s template=%s', $renderErr->getMessage(), $tplFile));
            return new Response($status, ['Content-Type' => 'text/plain; charset=utf-8', 'X-Quiote-Error-Type' => $e::class], $msg);
        }
        if ($body === '' || $body === false) {
            $body = Config::get('core.use_verbose_exception_template', false) ? 'Empty error template output' : 'Internal Server Error';
        }
        ...
        if (Config::get('core.use_verbose_exception_template', false) && ($status >= 400) && !str_contains($body, $e->getMessage())) {
            ...
            $body .= (str_ends_with($body, "\n") ? '' : "\n") . ($isPlain ? "Exception: " : '<div class="quiote-exception-message">') . $msgOut . ($isPlain ? '' : '</div>');
        }
        $headers = [];
        if (!headers_sent()) {
            $headers['Content-Type'] = (str_contains($tplFile, 'plaintext') ? 'text/plain' : 'text/html') . '; charset=utf-8';
        }
        $headers['X-Quiote-Error-Type'] = $e::class;

        \Quiote\Logging\Log::for($this)->debug(sprintf('[ErrorHandlingMiddleware] HTML response complete, status=%d length=%d', $status, strlen($body)));

        if ($status >= 500 && (\Quiote\Logging\Log::for($this)->isEnabled(\Quiote\Logging\Level::Debug))) {
            $snippet = substr($body, 0, 400);
            $snippetEsc = json_encode($snippet, JSON_UNESCAPED_SLASHES | JSON_PARTIAL_OUTPUT_ON_ERROR);
            \Quiote\Logging\Log::for($this)->debug(sprintf('[ErrorHandlingMiddleware] HTML response snippet: body_snippet=%s', $snippetEsc));
        }

        return new Response($status, $headers, $body);
    }

    private function extractContext(ServerRequestInterface $request): ?\Quiote\Context
    {
        // Attempt to recover context via global static or attribute on request
        // For now, rely on Context::getInstance() default profile.
        try {
            return \Quiote\Context::getInstance();
        } catch (\Throwable) {
            return null;
        }
    }
    ...
    public static function resolveTemplateFileStatic(ServerRequestInterface $request): string
    {
        ...
        $quioteDir = Config::get('core.quiote_dir') ?: (__DIR__ . '/../..');
        $baseDir = rtrim((string) $quioteDir, '/') . '/Exception/templates';
        $candidates = [];
        if ($forcePlain) {
            ...
            $ctxName = null;
            try {
                $ctxName = \Quiote\Context::getInstance()?->getName();
            } catch (\Throwable) {
            }
            if ($ctxName) {
            ...
            if (!$forceHtmlFallback && Config::has('exception.default_template')) {
                $def = Config::get('exception.default_template');
    ...
    private function resolveStructuredTemplate(string $format, string $mode): ?string
    {
        ...
        if ($format === 'html') {
            ...
            if (Config::has("exception.templates.html.$mode")) {
                $candidates[] = Config::get("exception.templates.html.$mode");
            }
        } elseif ($format === 'json') {
            if (Config::has("exception.templates.json.$mode")) {
                $candidates[] = Config::get("exception.templates.json.$mode");
            }
        }
        foreach ($candidates as $p) {
            if (is_string($p) && is_file($p) && is_readable($p)) {
    ...
    private function includeTemplateToResponse(string $file, int $status, ServerRequestInterface $request, Throwable $e, string $contentType): ResponseInterface
    {
        ...
        try {
            ...
            $msg = getenv('QUIOTE_DEBUG') ? 'Template render failed: ' . $renderErr->getMessage() : 'Internal Server Error';
            return new Response($status, ['Content-Type' => 'text/plain; charset=utf-8', 'X-Quiote-Error-Type' => $e::class], $msg);
        }
        return new Response($status, ['Content-Type' => $contentType . '; charset=utf-8', 'X-Quiote-Error-Type' => $e::class], $body ?: '');
    }
}
