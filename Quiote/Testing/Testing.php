...
class Testing
{
     ...
     * this same instance will be passed to PHPUnit in Testing::dispatch().
     * @return     PHP_CodeCoverage_Filter The code coverage filter for our tests.
     * @since      1.0.0
     */
    #[\Deprecated(message: 'Use PhpUnitCli', since: '1.1.0')]
	...
	{
		// Simple bootstrap that bypasses PhpUnitCli for PHPUnit 12 compatibility
		if($environment === null) {
		...
		if($environment === null) {
			// still no env? oh man...
			throw new QuioteException('You must supply an environment name to Testing::bootstrap() or set the name of the default environment to be used for testing in the configuration directive "testing.environment".');
		}
		...
		\Quiote\Quiote::bootstrap($environment);
			...
			switch($option[0]) {
				...
				case '--log-graphviz':
					if(PHPUnit_Util_Filesystem::fileExistsInIncludePath('Image/GraphViz.php')) {
						...
						throw new QuioteException('The Image_GraphViz package is not installed.');
					}
					break;
     ...
     * @throws     Execption if a dependency is missing
     * @since      1.0.0
     */
    #[\Deprecated(message: 'Use PhpUnitCli', since: '1.1.0')]
    protected static function checkCodeCoverageDeps()
	{
		if(extension_loaded('tokenizer') && extension_loaded('xdebug')) {
			...
			if(!extension_loaded('tokenizer')) {
				throw new QuioteException('The tokenizer extension is not loaded.');
			} else {
				throw new QuioteException('The Xdebug extension is not loaded.');
			}
		}
