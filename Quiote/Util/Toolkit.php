...
final class Toolkit
{
	...
	public static function clearCache($path = '')
	{
		...
		if(empty($baseCache)) {
			...
			// in the codebase (ConfigCache/CacheManager).
			$baseCache = rtrim(sys_get_temp_dir(), DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . 'quiote_cache';
		}
	...
	public static function floorDivide($numerator, $denominator, &$remainder)
	{
		if((int)$denominator != $denominator) {
			throw new QuioteException('Toolkit::floorDivive works only for int denominators');
		}
		...
		if (!isset(self::$moduleDirectiveCache[$cacheKey])) {
			...
			if ($rawDirective === null) {
				return Toolkit::expandVariables(
					Toolkit::expandDirectives($rawDirective),
					$variables
				);
			}
