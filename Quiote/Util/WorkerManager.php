...
/**
 * Centralized state management for any persistent worker runtime (see
 * {@see \Quiote\Runtime\Worker\WorkerRuntimeInterface}), ensuring
 * request-specific state is properly reset between requests while preserving
 * performance-critical cached data.
 * @since      1.0.0
 */
class WorkerManager
{
    ...
    /**
     * @var array{reset_count: int, initialization_time: float, db_connections_active: bool, apcu_acceleration: bool, start_time: float, last_reset_time: float}
     */
    ...
    $logger->debug('WorkerManager: Resetting context using getInstance approach');
        ...
        if (!empty($config['reset_config'])) {
            $preserveKeys = $config['preserve_config_keys'] ?? [];
            Config::resetWorkerState(is_array($preserveKeys) ? array_values(array_filter($preserveKeys, 'is_string')) : []);
        }
        ...
        if (class_exists(\Quiote\Routing\RoutingCallbackPool::class)) {
            RoutingCallbackPool::resetWorkerState(
                (bool) ($config['preserve_callback_pool'] ?? true),
                (bool) ($config['reset_stats'] ?? true)
            );
        }
        
        // Periodic deep cleanup. configure() takes arbitrary overrides, so this
        // is coerced rather than trusted -- and clamped, since 0 would divide by zero.
        $cleanupInterval = $config['max_requests_before_cleanup'] ?? 1000;
        $cleanupInterval = is_numeric($cleanupInterval) ? max(1, (int) $cleanupInterval) : 1000;
        if (self::$requestCount % $cleanupInterval === 0) {
