...
<xsl:stylesheet
	...
	xmlns:envelope_1_1="http://quiote.dev/quiote/config/global/envelope/1.1"
	xmlns:validators_1_0="http://quiote.dev/quiote/config/parts/validators/1.0"
	xmlns:validators_1_1="http://quiote.dev/quiote/config/parts/validators/1.1"
>
	<xsl:output method="xml" version="1.0" encoding="utf-8" indent="yes" />
	
	<xsl:include href="_common.xsl" />
	
	<xsl:variable name="validators_1_0" select="'http://quiote.dev/quiote/config/parts/validators/1.0'" />
	<xsl:variable name="validators_1_1" select="'http://quiote.dev/quiote/config/parts/validators/1.1'" />
	...
	<!-- we moved the default mappings ("string" => QuioteStringValidator with "min"=1 etc) from the config handler to that file -->
	<xsl:template match="envelope_0_11:configurations[not(@parent)]">
		<xsl:call-template name="_common-migrate-envelope-element">
			<xsl:with-param name="attributes"><dummy parent="%core.quiote_dir%/Config/defaults/validators.xml" /></xsl:with-param>
		</xsl:call-template>
	</xsl:template>
	...
	<xsl:template match="validators_1_0:validator[validators_1_0:arguments[@base]]/@provides">
		<xsl:attribute name="provides"><xsl:value-of select="php:function('QuioteDependencyManager::populateArgumentBaseKeyRefs', concat(../validators_1_0:arguments/@base,'[',.,']'))" /></xsl:attribute>
	</xsl:template>
	...
	<xsl:template match="validators_1_0:validator[validators_1_0:arguments[@base]]/@depends[substring(.,1,1)='[']">
		<xsl:attribute name="depends"><xsl:value-of select="php:function('QuioteDependencyManager::populateArgumentBaseKeyRefs', concat(../validators_1_0:arguments/@base,.))" /></xsl:attribute>
	</xsl:template>
