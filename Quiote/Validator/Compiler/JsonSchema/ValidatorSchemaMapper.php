 ...
 * JSON Schema object describing the request parameters an action accepts --
 * one validator declaration drives HTTP validation, the `inputSchema` of an
 * action exposed as an MCP tool, and the parameters/requestBody of an
 * OpenAPI operation alike.
 *
 ...
 * true` and the real enforcement still happens when the request is dispatched
 * through the pipeline (the same validators run). So where a rule
...
final class ValidatorSchemaMapper
{
     ...
     *        via {@see \Quiote\Execution\HttpMethodMapper}) the schema is being
    ...
    private function number(array $params): array
    {
        ...
        $isInteger = is_string($type) && in_array(strtolower($type), ['int', 'integer'], true);
    ...
    private function enum(array $params): array
    {
        ...
        if (is_string($values)) {
            $configured = $params['sep'] ?? null;
            $separator = is_string($configured) ? $configured : ',';
