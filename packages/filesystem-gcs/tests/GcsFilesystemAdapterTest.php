...
final class FakeGcsFilesystemTransport implements ClientInterface
{
    ...
    /**
     * A single-page fake of the XML interoperability API's GET Bucket: real pagination is
     * exercised at the GcsClient level, this only needs prefix/delimiter grouping for the
     * filesystem adapter's listContents().
     */
    private function handleList(string $query): ResponseInterface
    {
        ...
        $xml = "<?xml version=\"1.0\" encoding=\"UTF-8\"?><ListBucketResult>{$contentsXml}{$prefixesXml}<IsTruncated>false</IsTruncated></ListBucketResult>";
