 ...
 * of {@see \Quiote\Middleware\MiddlewarePipeline} hardcoding them (see
 * docs/PLUGIN_EXTRACTION_PLAN.md §2.3). Both middleware still carry their own
 * `#[Middleware(...)]` attribute for ordering — this plugin only supplies the
 * per-context factory ({@see PluginRegistrar::attributedMiddleware()}) so each
 * middleware gets *this* pipeline's own `Controller`, not a container-autowired
 * (and possibly unrelated, for apps with a custom Controller subclass) one.
 *
 * Physically split into its own package, `packages/csrf/` (developed in-tree,
 * symlinked via a path repository; see docs/MONOREPO_SPLIT_PLAN.md) — not yet
 * pushed to a standalone repo, and `Quiote::bootstrap()` still runs this
