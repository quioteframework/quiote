...
class OidcClientTest extends TestCase
{
	...
	/**
	 * The `nonce` goes through the injected RandomnessInterface seam rather
	 * than a direct random_bytes() call, per §6.2 of the record/replay
	 * determinism plan -- so a replay engine can reproduce exactly which
	 * nonce a recorded authorization request carried. `state` and the PKCE
	 * verifier are generated by the underlying league/oauth2-client provider
	 * and are out of scope for this seam.
	 */
	public function testNonceIsDerivedFromTheInjectedRandomnessSeam(): void
	{
		$client = new OidcClient(
			'id',
			'secret',
		...
		$this->assertSame(bin2hex((new SeededRandomness(42))->bytes(16)), $request->getState()->getNonce());
	}

	public function testSameSeedProducesTheSameNonceAcrossTwoClients(): void
	{
		$makeClient = fn() => new OidcClient(
			'id',
			'secret',
			'https://app.example.com/callback',
			'https://idp.example.com/authorize',
		...
		$nonceA = $makeClient()->buildAuthorizationRequest()->getState()->getNonce();
		$nonceB = $makeClient()->buildAuthorizationRequest()->getState()->getNonce();

		$this->assertSame($nonceA, $nonceB);
	}
