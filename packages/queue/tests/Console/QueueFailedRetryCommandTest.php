...
final class QueueFailedRetryCommandTestJob implements Job
{
    /** @var list<int> */
    ...
    {
        self::$handled[] = $this->userId;
    }
}

final class QueueFailedRetryCommandFakeStore implements InspectableFailedJobStoreInterface
{
...
final class QueueFailedRetryCommandTest extends PhpUnitTestCase
{
    #[Before]
    #[After]
    public function resetQueueState(): void
    {
        ...
        PluginManager::reset();
        QueueFailedRetryCommandTestJob::$handled = [];
    ...
    private function record(string $id, int $userId): FailedJobRecord
    {
        return new FailedJobRecord(
            id: $id,
            jobClass: QueueFailedRetryCommandTestJob::class,
    ...
    private function tester(QueueFailedRetryCommandFakeStore $store): CommandTester
    {
        ...
        return new CommandTester(new QueueFailedRetryCommand());
    }

    public function testErrorsWhenNeitherIdNorAllIsGiven(): void
    {
        $exitCode = $this->tester(new QueueFailedRetryCommandFakeStore())->execute([]);
    ...
    {
        $exitCode = $this->tester(new QueueFailedRetryCommandFakeStore())->execute(['id' => 'missing']);

        $this->assertSame(1, $exitCode);
    }

    public function testRetriesASingleJobAndRemovesItFromTheStore(): void
    {
        $store = new QueueFailedRetryCommandFakeStore();
        $store->records['abc'] = $this->record('abc', 42);

        $exitCode = $this->tester($store)->execute(['id' => 'abc']);

        $this->assertSame(0, $exitCode);
        $this->assertSame([42], QueueFailedRetryCommandTestJob::$handled);
        $this->assertNull($store->find('abc'));
    }

    public function testRetryAllRetriesEveryFailedJob(): void
    {
        $store = new QueueFailedRetryCommandFakeStore();
        $store->records['a'] = $this->record('a', 1);
        $store->records['b'] = $this->record('b', 2);

        $exitCode = $this->tester($store)->execute(['--all' => true]);

        $this->assertSame(0, $exitCode);
        $this->assertSame([1, 2], QueueFailedRetryCommandTestJob::$handled);
