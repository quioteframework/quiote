...
use Agavi\Validator\AgaviValidationReportQuery;
 ...
 */
class AgaviAdditionalValidatorEdgeCasesTest extends AgaviUnitTestCase
{
    ...
    public function testReportQueryMinMaxAndByErrorName(): void
    {
        ...
        $vNotice = $vm->createValidator(Agavi\Validator\AgaviStringValidator::class, ['a'], ['min'=>'too short'], ['name'=>'vNotice','min'=>5,'severity'=>'notice']); // will fail 'min'
        $vError  = $vm->createValidator(Agavi\Validator\AgaviStringValidator::class, ['b'], ['max'=>'too long'], ['name'=>'vError','max'=>1,'severity'=>'error']); // will fail 'max'
        $vCritical = $vm->createValidator(Agavi\Validator\AgaviStringValidator::class, ['c'], ['min'=>'too short'], ['name'=>'vCritical','min'=>10,'severity'=>'critical']); // fail 'min'
        $req = $this->newWebRequest(['a'=>'xx','b'=>'abc','c'=>'short']);
        $this->assertFalse($vm->execute($req));

        $q = new AgaviValidationReportQuery($vm->getReport());
        $this->assertCount(3, $vm->getReport()->getIncidents());

        // Min severity ERROR should include error + critical (exclude notice)
        $sevFiltered = $q->byMinSeverity(AgaviValidator::ERROR);
        $this->assertCount(2, $sevFiltered->getIncidents());
        // Max severity ERROR removes the critical one leaving only error severity
        $errorOnly = $sevFiltered->byMaxSeverity(AgaviValidator::ERROR);
    ...
    public function testRegexValidatorFailureAndSubpatternExport(): void
    {
        ...
        $rvFail = $vm->createValidator(Agavi\Validator\AgaviRegexValidator::class, ['code'], [''=> 'regex fail'], ['name'=>'rxFail','pattern'=>'/^[0-9]+$/','match'=>1]);
        ...
        $rvOk = $vm2->createValidator(Agavi\Validator\AgaviRegexValidator::class, ['val'], [], [
            'name'=>'rxOk',
    ...
    {
        $vm = $this->vm(['mode'=>AgaviValidationManager::MODE_RELAXED]);
        $em = $vm->createValidator(Agavi\Validator\AgaviEmailValidator::class, ['email'], [''=> 'email invalid'], ['name'=>'em','severity'=>'error']);
        $req = $this->newWebRequest(['email'=>'not-an-email']);
        $this->assertFalse($vm->execute($req));
        // Non-scalar value triggers throwError branch
        $vm2 = $this->vm(['mode'=>AgaviValidationManager::MODE_RELAXED]);
        $em2 = $vm2->createValidator(Agavi\Validator\AgaviEmailValidator::class, ['e'], [''=> 'email invalid'], ['name'=>'em2','severity'=>'error']);
    ...
    {
        $vm = $this->vm(['mode'=>AgaviValidationManager::MODE_RELAXED]);
        $sv = $vm->createValidator(Agavi\Validator\AgaviStringValidator::class, ['s'], ['min'=>'too short','max'=>'too long'], ['name'=>'sv','min'=>3,'max'=>5,'severity'=>'error','trim'=>true,'export'=>'s_out']);
        // Too short after trim
        $req1 = $this->newWebRequest(['s'=>'  a ']);
        $this->assertFalse($vm->execute($req1));
        // Reset for next attempt
        $vm2 = $this->vm(['mode'=>AgaviValidationManager::MODE_RELAXED]);
        $sv2 = $vm2->createValidator(Agavi\Validator\AgaviStringValidator::class, ['s'], ['min'=>'too short','max'=>'too long'], ['name'=>'sv2','min'=>1,'max'=>4,'severity'=>'error','trim'=>true,'export'=>'s_out']);
        ...
        $sv3 = $vm3->createValidator(Agavi\Validator\AgaviStringValidator::class, ['s'], ['max'=>'too long'], ['name'=>'sv3','max'=>2,'severity'=>'error']);
        $req3 = $this->newWebRequest(['s'=>'abcd']);
        $this->assertFalse($vm3->execute($req3));
        // Non-scalar
        $vm4 = $this->vm(['mode'=>AgaviValidationManager::MODE_RELAXED]);
        $sv4 = $vm4->createValidator(Agavi\Validator\AgaviStringValidator::class, ['s'], [], ['name'=>'sv4','severity'=>'error']);
    ...
    {
        $vm = $this->vm(['mode'=>AgaviValidationManager::MODE_RELAXED]);
        $num = $vm->createValidator(Agavi\Validator\AgaviNumberValidator::class, ['n'], ['type'=>'not number'], ['name'=>'num','type'=>'int','severity'=>'error']);
        $req = $this->newWebRequest(['n'=>'12x']);
        $this->assertFalse($vm->execute($req));
        // Valid integer for comparison
        $vm2 = $this->vm(['mode'=>AgaviValidationManager::MODE_RELAXED]);
        $num2 = $vm2->createValidator(Agavi\Validator\AgaviNumberValidator::class, ['n'], [], ['name'=>'num2','type'=>'int']);
    ...
    {
        \Agavi\Config\AgaviConfig::set('core.use_translation', true);
        $vm = $this->vm(['mode'=>AgaviValidationManager::MODE_RELAXED]);
        // Invalid format (expects yyyy-MM-dd HH:mm:ss)
        $dtInvalid = $vm->createValidator(Agavi\Validator\AgaviDateTimeValidator::class, ['ts'], ['format'=>'bad format'], [
        ...
        $vm2 = $this->vm(['mode'=>AgaviValidationManager::MODE_RELAXED]);
        $dtBounds = $vm2->createValidator(Agavi\Validator\AgaviDateTimeValidator::class, ['ts'], ['min'=>'too early','max'=>'too late'], [
        ...
        $vm3 = $this->vm(['mode'=>AgaviValidationManager::MODE_RELAXED]);
        $dtBounds2 = $vm3->createValidator(Agavi\Validator\AgaviDateTimeValidator::class, ['ts'], ['max'=>'too late'], [
        ...
        $vm4 = $this->vm(['mode'=>AgaviValidationManager::MODE_RELAXED]);
        $dtOk = $vm4->createValidator(Agavi\Validator\AgaviDateTimeValidator::class, ['ts'], [], [
    ...
    public function testNumberValidatorLocaleThousandParsing(): void
    {
        ...
        \Agavi\Config\AgaviConfig::set('core.use_translation', true);
        $vm = $this->vm(['mode'=>AgaviValidationManager::MODE_RELAXED]);
        $num = $vm->createValidator(Agavi\Validator\AgaviNumberValidator::class, ['price'], [], [
        ...
        $vm2 = $this->vm(['mode'=>AgaviValidationManager::MODE_RELAXED]);
        $num2 = $vm2->createValidator(Agavi\Validator\AgaviNumberValidator::class, ['price'], [], [
        ...
        $vm3 = $this->vm(['mode'=>AgaviValidationManager::MODE_RELAXED]);
        $num3 = $vm3->createValidator(Agavi\Validator\AgaviNumberValidator::class, ['price'], ['type'=>'bad'], [
