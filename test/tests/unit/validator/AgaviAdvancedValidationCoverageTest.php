 ...
 */
class AgaviAdvancedValidationCoverageTest extends AgaviUnitTestCase
{
    ...
    $q = new Agavi\Validator\AgaviValidationReportQuery($rep);
    ...
    public function testDependsMissingSkipsValidator(): void
    {
        ...
        // AgaviValidator::execute() returned NOT_PROCESSED (-1) which the manager treats as "did not change success".
        ...
        $query = new AgaviValidationReportQuery($vm->getReport());
        ...
        $this->assertSame(AgaviValidator::SUCCESS, $vm->getReport()->getResult(), 'Report result should remain SUCCESS when only validator is skipped');
    }
    ...
    public function testEqualsStrictVersusNonStrict(): void
    {
        ...
        $eq1 = $vm1->createValidator(Agavi\Validator\AgaviEqualsValidator::class, ['n'], [], ['name'=>'eq1','value'=>5,'strict'=>false]);
        $req1 = $this->newWebRequest(['n'=>'5']);
        $this->assertTrue($vm1->execute($req1));
        // Strict: '5' !== 5
        $vm2 = $this->vm(['mode'=>AgaviValidationManager::MODE_RELAXED]);
        $eq2 = $vm2->createValidator(Agavi\Validator\AgaviEqualsValidator::class, ['n'], [], ['name'=>'eq2','value'=>5,'strict'=>true]);
    ...
    public function testNumberValidatorLocaleParsing(): void
    {
        ...
        $vm = $this->vm(['mode'=>AgaviValidationManager::MODE_RELAXED]);
        $num = $vm->createValidator(Agavi\Validator\AgaviNumberValidator::class, ['price'], [], ['name'=>'num','type'=>'float','cast_to'=>'float']);
        ...
        $vm2 = $this->vm(['mode'=>AgaviValidationManager::MODE_RELAXED]);
        $num2 = $vm2->createValidator(Agavi\Validator\AgaviNumberValidator::class, ['price'], [], ['name'=>'num2','type'=>'float','cast_to'=>'float']);
