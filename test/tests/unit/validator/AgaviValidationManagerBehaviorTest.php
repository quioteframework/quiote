...
/**
 * Behavior-focused tests for AgaviValidationManager: dependency token propagation,
 * short-circuit on CRITICAL, strict mode parameter pruning, and manual incident aggregation.
 */
class AgaviValidationManagerBehaviorTest extends AgaviUnitTestCase
{
    private function newVm(array $params = []): AgaviValidationManager
    {
    ...
    {
        $vm = $this->newVm(['mode' => AgaviValidationManager::MODE_RELAXED]);
    ...
    public function testManualIncidentAggregation(): void
    {
        ...
        $arg = new AgaviValidationArgument('foo');
        $incident->addError(new AgaviValidationError('msg1', null, [$arg]));
