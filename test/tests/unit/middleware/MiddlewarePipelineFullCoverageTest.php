...
class MiddlewarePipelineFullCoverageTest extends TestCase
{
    ...
    public function testPipelineBuildAndReuseAndReset()
    {
        ...
        $req = $this->makeReq();
        // First call builds
        try { $pipeline->handle($req); } catch (\Throwable $e) { /* ignore terminal if reached */ }
    ...
    public function testOptionalMiddlewaresToggledIndependentlyAndCollectively()
    {
        ...
        $p1 = new MiddlewarePipeline($ctx); $p1->handle($this->makeReq()); $d1 = $p1->debugStack();
        ...
        $p2 = new MiddlewarePipeline($ctx); $p2->handle($this->makeReq()); $d2 = $p2->debugStack();
        ...
        $p3 = new MiddlewarePipeline($ctx); $p3->handle($this->makeReq()); $d3 = $p3->debugStack();
        ...
        $p4 = new MiddlewarePipeline($ctx); $p4->handle($this->makeReq()); $d4 = $p4->debugStack();
    ...
    public function testErrorHandlingStatusMappingsAndCorrelationHeaders()
    {
        ...
        // 400 mapping
        $r400 = $eh->process($this->makeReq(['Accept' => 'text/plain', 'Correlation-Id' => 'abc123']), new class implements RequestHandlerInterface { public function handle(ServerRequestInterface $r): ResponseInterface { throw new InvalidArgumentException('bad arg'); } });
        ...
        $r422 = $eh->process($this->makeReq(['X-Correlation-ID' => 'legacy-id']), new class implements RequestHandlerInterface { public function handle(ServerRequestInterface $r): ResponseInterface { throw new DomainException('bad domain'); } });
        $this->assertSame(422, $r422->getStatusCode());
        // 500 generic
        $r500 = $eh->process($this->makeReq(), new class implements RequestHandlerInterface { public function handle(ServerRequestInterface $r): ResponseInterface { throw new RuntimeException('explode'); } });
    ...
    public function testErrorHandlingJsonNegotiationViaAcceptAndOutputType()
    {
        $eh = new ErrorHandlingMiddleware();
        // Accept header
        $jsonResp1 = $eh->process($this->makeReq(['Accept' => 'application/json']), new class implements RequestHandlerInterface { public function handle(ServerRequestInterface $r): ResponseInterface { throw new RuntimeException('jsonfail'); } });
        ...
        $jsonResp2 = $eh->process($this->makeReq([], ['output_type' => 'json']), new class implements RequestHandlerInterface { public function handle(ServerRequestInterface $r): ResponseInterface { throw new RuntimeException('ot-json'); } });
    ...
    public function testErrorHandlingProductionModeMasksMessage()
    {
        ...
        $eh = new ErrorHandlingMiddleware();
        $resp = $eh->process($this->makeReq(['Accept' => 'application/json']), new class implements RequestHandlerInterface { public function handle(ServerRequestInterface $r): ResponseInterface { throw new RuntimeException('sensitive'); } });
    ...
    public function testErrorTemplateFallbackOnIncludeFailure()
    {
        ...
        $eh = new ErrorHandlingMiddleware();
        $resp = $eh->process($this->makeReq(), new class implements RequestHandlerInterface { public function handle(ServerRequestInterface $r): ResponseInterface { throw new RuntimeException('tmpl'); } });
        $this->assertSame(500, $resp->getStatusCode());
    ...
    public function testTerminalSentinelThrowsWhenNoResponse()
    {
        ...
        $testPipeline = new class($ctx) extends MiddlewarePipeline {
            ...
            public function forceBuildOnlySentinel(): void {
                ...
                $builtProp = $ref->getProperty('built'); $builtProp->setAccessible(true);
                $handlerProp = $ref->getProperty('handler'); $handlerProp->setAccessible(true);
                $debugProp = $ref->getProperty('debugStack'); $debugProp->setAccessible(true);
                ...
                $sentinel = new class implements \Psr\Http\Server\MiddlewareInterface {
                    public function process(ServerRequestInterface $request, RequestHandlerInterface $handler): ResponseInterface { throw new RuntimeException('Terminal pipeline reached without response'); }
                };
                $relay = new Relay\Relay([$sentinel]);
                $handlerProp->setValue($this, new class($relay) implements RequestHandlerInterface { public function __construct(private Relay\Relay $relay) {} public function handle(ServerRequestInterface $r): ResponseInterface { return $this->relay->handle($r);} });
