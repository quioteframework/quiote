...
use Agavi\Logging\Log;
use Agavi\Logging\Level;
use Agavi\Logging\LogContext;
use Agavi\Logging\LogRegistry;
use Agavi\Logging\Sink\JsonStdoutSink;
...
class LoggingTest extends TestCase
{
    ...
    public function testLongestPrefixWins(): void
    {
        ...
        Log::setLevels([
            'Agavi'         => Level::Warning,
            'Agavi.Routing' => Level::Debug,
            'App.Orders'    => Level::Debug,
        ]);
        $this->assertSame(Level::Debug,   LogRegistry::resolveLevel('Agavi.Routing'));
        $this->assertSame(Level::Debug,   LogRegistry::resolveLevel('Agavi.Routing.Matcher')); // inherits prefix
        $this->assertSame(Level::Warning, LogRegistry::resolveLevel('Agavi.Security'));
        $this->assertSame(Level::Info,    LogRegistry::resolveLevel('App.Misc'));               // default
        // "Agavi" prefix must NOT match a different top-level segment.
        $this->assertSame(Level::Info,    LogRegistry::resolveLevel('AgaviExtras.Thing'));
    }
    ...
    {
        Log::setDefaultLevel(Level::Info);
        Log::setLevel('Agavi', Level::Warning);
        Log::addSink($this->sink(Level::Debug));

        $routing = Log::create('Agavi.Routing'); // Warning
