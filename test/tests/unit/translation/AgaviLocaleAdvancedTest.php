...
class AgaviLocaleAdvancedTest extends TestCase
{
    private function makeLocale(string $identifier, array $data = [], array $params = []): AgaviLocale
    {
        $loc = new AgaviLocale();
    ...
    {
        $parsed = AgaviLocale::parseLocaleIdentifier($id);
        foreach ($exp as $k => $v) {
    ...
    public function testParseLocaleIdentifierInvalid(string $id)
    {
        ...
        AgaviLocale::parseLocaleIdentifier($id);
    }
    ...
    {
        $this->assertSame(['en_US', 'en'], AgaviLocale::getLookupPath('en_US'));
        // Actual order from implementation places script-only path before territory path of base language
        $this->assertSame(['zh_Hant_TW', 'zh_Hant', 'zh_TW', 'zh'], AgaviLocale::getLookupPath('zh_Hant_TW'));
    }

    public function testGetLookupPathZhHantTwActual()
    {
        $this->assertSame(['zh_Hant_TW', 'zh_Hant', 'zh_TW', 'zh'], AgaviLocale::getLookupPath('zh_Hant_TW'));
        $this->assertSame(['en_POSIX', 'en'], AgaviLocale::getLookupPath('en_POSIX'));
        $this->assertSame(['sr_Cyrl_RS_REVISED', 'sr_Cyrl_RS', 'sr_Cyrl', 'sr_RS_REVISED', 'sr_RS', 'sr'], AgaviLocale::getLookupPath('sr_Cyrl_RS_REVISED'));
    }

    public function testGetTimeZoneOptionString()
    {
        $dt = new DateTime('2025-01-01 00:00:00', new DateTimeZone('Europe/Berlin'));
        $this->assertSame('@timezone=Europe/Berlin', AgaviLocale::getTimeZoneOptionString($dt));
        $tz = new DateTimeZone('UTC');
        $this->assertSame('@timezone=UTC', AgaviLocale::getTimeZoneOptionString($tz));
        $this->assertSame('@timezone=UTC', AgaviLocale::getTimeZoneOptionString(time()));
        $this->assertSame('@timezone=GMT+02:00', AgaviLocale::getTimeZoneOptionString('+02:00'));
        $this->assertSame('', AgaviLocale::getTimeZoneOptionString(''));
        $this->assertSame(';timezone=UTC', AgaviLocale::getTimeZoneOptionString('UTC', ';'));
    }
