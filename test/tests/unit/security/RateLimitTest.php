...
use Agavi\Security\RateLimit\LoginThrottle;
use Agavi\Security\RateLimit\PdoRateLimiterStorage;
...
final class RateLimitTest extends TestCase
{
    ...
    public function testPdoPurgeExpiredRemovesStaleRows(): void
    {
        ...
        $this->assertSame(1, (int) $pdo->query('SELECT COUNT(*) FROM agavi_rate_limit')->fetchColumn());

        // Force the row to be expired, then purge.
        $pdo->exec('UPDATE agavi_rate_limit SET expires_at = ' . (time() - 10));
        $deleted = $storage->purgeExpired();
        $this->assertSame(1, $deleted);
        $this->assertSame(0, (int) $pdo->query('SELECT COUNT(*) FROM agavi_rate_limit')->fetchColumn());
    }
