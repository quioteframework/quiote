...
/**
 * Symfony CSRF TokenStorage backed by Agavi's session storage.
 *
 * Lets symfony/security-csrf persist its per-session tokens through whatever
 * AgaviStorage the context provides (native session, PDO, etc.) instead of the
 ...
 * @subpackage security
 */
final class AgaviSessionTokenStorage implements TokenStorageInterface
{
    /** Namespace prefix for CSRF token keys in the session. */
    private const PREFIX = 'org.agavi.csrf.';
