...
class SlotDispatcher
{
    ...
    public function dispatch(ServerRequestInterface $parentRequest, string $module, string $action, array $parameters = [], ?string $outputType = null): string
    {
        ...
        try {
            ...
            if (!($actionInstance instanceof AgaviAction)) {
                throw new AgaviException('Slot action did not resolve to AgaviAction');
            }
            ...
            if (!$rdh) {
                ...
                if (!($rdh instanceof AgaviWebRequest)) {
                    throw new \RuntimeException('Canonical AgaviWebRequest missing in SlotDispatcher::dispatch (simple)');
                }
            }
            if ($cacheEnabled) {
            ...
            if ($actionInstance->isSimple()) {
                ...
                if (!($rd instanceof AgaviWebRequest)) {
                    throw new \RuntimeException('Canonical AgaviWebRequest missing in SlotDispatcher simple action path');
                }
                ...
                if ($viewCanonical !== AgaviView::NONE) {
                    ...
                    if (!$viewInstance) {
                        ...
                        if ($viewInstance) {
                            try {
                                $vic = new \Agavi\Execution\ImmutableViewInitContext($this->controller->getContext(), $viewModule, $viewCanonical, strtolower(($outputType ?? $this->controller->getOutputType()->getName())), $module, $action, (array)$attributeSnapshot, $this->controller->getGlobalResponse());
                ...
                if (!($rd instanceof AgaviWebRequest)) {
                    throw new \RuntimeException('Canonical AgaviWebRequest missing in SlotDispatcher non-simple action path');
                }
                ...
                if (!$vres->ok) {
                    ...
                    if ($vn !== AgaviView::NONE) {
                        ...
                        if ($viewInstance) {
                            try {
                                $vic = new \Agavi\Execution\ImmutableViewInitContext($this->controller->getContext(), $vm, $vn, strtolower(($outputType ?? $this->controller->getOutputType()->getName())), $module, $action, method_exists($actionInstance, 'getAttributes') ? (array)$actionInstance->getAttributes() : [], $this->controller->getGlobalResponse());
                                $viewInstance->initialize($vic);
                            } catch (\Throwable) {
                            }
                        }
                ...
                if ($vn !== AgaviView::NONE) {
                    ...
                    if ($viewInstance) {
                        try {
                            $vic = new \Agavi\Execution\ImmutableViewInitContext($this->controller->getContext(), $vm, $vn, strtolower(($outputType ?? $this->controller->getOutputType()->getName())), $module, $action, $attrs, $this->controller->getGlobalResponse());
                            $viewInstance->initialize($vic);
                        } catch (\Throwable) {
                        }
                    }
    ...
    public function dispatchWithContext(ServerRequestInterface $parentRequest, string $module, string $action, array $parameters = [], ?string $outputType = null): ActionExecutionContext
    {
        ...
        if (!($sharedRequest instanceof AgaviWebRequest)) {
            throw new \RuntimeException('Canonical AgaviWebRequest missing in SlotDispatcher::dispatchWithContext fallback');
        }
