...
class PsrServerRequestAdapter implements ServerRequestInterface
{
    ...
    public function getLegacyRequest(): AgaviRequest { return $this->legacy; }

    private function cloneSelf(): self { return clone $this; }
    ...
    public function withCookieParams(array $cookies): static { $c = $this->cloneSelf(); $c->cookieParams = $cookies; return $c; }
    public function getQueryParams(): array { return $this->queryParams; }
    public function withQueryParams(array $query): static { $c = $this->cloneSelf(); $c->queryParams = $query; return $c; }
    public function getUploadedFiles(): array { return $this->uploadedFiles; }
    public function withUploadedFiles(array $uploadedFiles): static { $c = $this->cloneSelf(); $c->uploadedFiles = $uploadedFiles; return $c; }
    public function getParsedBody(): mixed { return $this->parsedBody; }
    public function withParsedBody($data): static { if(!is_array($data) && $data !== null) throw new InvalidArgumentException('parsed body must be array|null'); $c = $this->cloneSelf(); $c->parsedBody = $data ?? []; return $c; }
    public function getAttributes(): array { return $this->attributes; }
    public function getAttribute($name, $default = null): mixed { return $this->attributes[$name] ?? $default; }
    public function withAttribute($name, $value): static { $c = $this->cloneSelf(); $c->attributes[$name] = $value; return $c; }
    public function withoutAttribute($name): static { $c = $this->cloneSelf(); unset($c->attributes[$name]); return $c; }

    // ----- MessageInterface -----
    public function getProtocolVersion(): string { return $this->protocolVersion; }
    public function withProtocolVersion($version): static { $c = $this->cloneSelf(); $c->protocolVersion = $version; return $c; }
    ...
    public function withHeader($name, $value): static { $c = $this->cloneSelf(); $c->headers[strtolower($name)] = is_array($value)? array_values($value):[$value]; return $c; }
    public function withAddedHeader($name, $value): static { $c = $this->cloneSelf(); $ln = strtolower($name); $add = is_array($value)? $value:[$value]; $c->headers[$ln] = array_merge($c->headers[$ln]??[], $add); return $c; }
    public function withoutHeader($name): static { $c=$this->cloneSelf(); unset($c->headers[strtolower($name)]); return $c; }
    public function getBody(): StreamInterface { return $this->body; }
    public function withBody(StreamInterface $body): static { $c=$this->cloneSelf(); $c->body=$body; return $c; }

    // ----- RequestInterface -----
    public function getRequestTarget(): string { return $this->uri->getPath() ?: '/'; }
    public function withRequestTarget($requestTarget): static { $c=$this->cloneSelf(); /* ignore mutability of target */ return $c; }
    public function getMethod(): string { return $this->method; }
    public function withMethod($method): static { $c=$this->cloneSelf(); $c->method=strtoupper($method); return $c; }
    public function getUri(): UriInterface { return $this->uri; }
    public function withUri(UriInterface $uri, $preserveHost = false): static { $c=$this->cloneSelf(); $c->uri=$uri; if(!$preserveHost && $uri->getHost()) $c->headers['host']=[$uri->getHost()]; return $c; }
