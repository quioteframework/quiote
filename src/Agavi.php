...
AgaviConfig::set('exception.default_template', $agavi_config_directive_core_agavi_dir . '/Exception/templates/plaintext.php');
...
final class Agavi
{
		...
		try {
			...
			foreach($contextList as $ctxName) {
				...
				try {
					...
					\Agavi\Logging\Log::create('Agavi.Agavi')->debug('bootstrap controller prime failed for context ' . $ctxName . ': ' . $e->getMessage());
				}
			}
	...
	public static function prewarm(?string $context = null): void
	{
		...
		try {
			...
			\Agavi\Logging\Log::create('Agavi.Agavi')->warning('prewarm failed: '.$e->getMessage());
		}
	}
	...
	public static function context(?string $name = null, bool $prime = false): AgaviContext
	{
		...
		if($prime) {
			try {
				...
				\Agavi\Logging\Log::create('Agavi.Agavi')->debug('context prime failed: '.$e->getMessage());
			}
		}
		return $ctx;
	}
