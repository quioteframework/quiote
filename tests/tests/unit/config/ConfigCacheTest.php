...
class ConfigCacheTest extends PhpUnitTestCase
{
	...
	/**
	 * writeCacheFile() takes the handler's declaration and serializes it; what lands on disk is a PHP
	 * file that returns exactly that value and names where it came from.
	 */
	public function testWriteCacheFile(): void
	{
		$declaration = ['core.app_name' => 'cache test', 'nested' => [1, 2, 3]];
		...
		ConfigCache::writeCacheFile($config, $cacheName, $declaration, 'TestHandler');

		$this->assertFileExists($cacheName);
		$this->assertSame($declaration, include $cacheName);
		$content = file_get_contents($cacheName);
		$this->assertIsString($content);
		$this->assertStringContainsString('// Compiled from: ' . $config, $content);
		$this->assertStringContainsString('// Generated by: TestHandler', $content);
	}

	/**
	 * A cache entry can only ever hand back data, so a declaration carrying something no PHP literal
	 * can express is refused at the write -- where the source file can still be named -- instead of
	 * producing a file that cannot reproduce it.
	 */
	public function testWriteCacheFileRefusesADeclarationThatIsNotData(): void
	{
		$config = Config::getString('core.config_dir').DIRECTORY_SEPARATOR.'foo.xml';

		$this->expectException(\Quiote\Exception\CacheException::class);
		$this->expectExceptionMessageMatches('/must be data a PHP literal can express/');
		ConfigCache::writeCacheFile($config, ConfigCache::getCacheName($config), ['handler' => new \stdClass()], 'TestHandler');
	}

	/**
	 * The config cache holds PHP that this process include()s, and
	...
	public function testCreatedCacheDirectoryIsNeverWorldWritable(): void
	{
		...
		try {
			...
			ConfigCache::writeCacheFile($config, ConfigCache::getCacheName($config), ['test' => true]);
