<?php

// Bootstrap file for PHPUnit 12 testing

// Load Composer autoloader first
require_once(__DIR__ . '/../vendor/autoload.php');

// Set up basic configuration - MUST HAPPEN BEFORE BOOTSTRAP
$testDir = realpath(__DIR__);
$appDir = realpath(__DIR__ . '/sandbox/app/');
$srcDir = realpath(__DIR__ . '/../Quiote/');
// Ensure cache directory exists
if (!is_dir($appDir . '/cache')) {
    mkdir($appDir . '/cache', 0777, true);
}
\Quiote\Config\Config::set('core.testing_dir', $testDir);
\Quiote\Config\Config::set('core.app_dir', $appDir);
\Quiote\Config\Config::set('core.config_dir', $appDir . '/Config/');
\Quiote\Config\Config::set('core.cache_dir', $appDir . '/cache');
\Quiote\Config\Config::set('core.system_config_dir', $srcDir . '/Config/defaults/');
// Mirror the filesystem directives Quiote::bootstrap() sets. Some tests extend
// plain PHPUnit\Framework\TestCase (not PhpUnitTestCase) and never call
// Quiote::bootstrap(), yet still touch Context (e.g. via the debug logger),
// which lazily compiles config_handlers.xml. The handler patterns there contain
// %core.module_dir% etc.; if those directives are unset at compile time the
// placeholders are baked literally into the shared static handler map and never
// match, poisoning every later test in the process. Defining them up-front (and
// not read-only, so a later Quiote::bootstrap() can still set its own) guarantees
// the placeholders always resolve regardless of test execution order.
\Quiote\Config\Config::set('core.module_dir', $appDir . '/Modules');
\Quiote\Config\Config::set('core.model_dir', $appDir . '/Models');
\Quiote\Config\Config::set('core.lib_dir', $appDir . '/Lib');
\Quiote\Config\Config::set('core.template_dir', $appDir . '/Templates');
\Quiote\Config\Config::set('core.default_context', 'testing');
\Quiote\Config\Config::set('core.environment', 'testing');
\Quiote\Config\Config::set('testing.environment', 'testing');
// Disable CSRF enforcement in the test environment (as Symfony and others do):
// otherwise every POST/PUT/DELETE functional test would need to carry a valid
// token. The dedicated CsrfTest re-enables it per test to exercise the feature.
\Quiote\Config\Config::set('core.csrf.enabled', false);
// Set the namespace prefix for the test environment before any bootstrapping
\Quiote\Config\Config::set('core.namespace_prefix', 'Sandbox');

// Required for both autoloaded and non-autoloaded classes
// Only load Quiote.php if we're not in an isolated test
// Isolated tests (like ConfigTest) will load what they need directly
if (!isset($_ENV['QUIOTE_ISOLATED_TEST']) && !getenv('QUIOTE_ISOLATED_TEST')) {
    require_once(__DIR__ . '/../Quiote/Quiote.php');
}

// Simple bootstrap approach for PHPUnit 12
// Test cases will handle bootstrapping themselves either:
// 1. Via Testing::bootstrap() for normal tests
// 2. Via their setUp() method for isolated tests with RunInSeparateProcess attributes

// Do not bootstrap here as it interferes with isolation tests
