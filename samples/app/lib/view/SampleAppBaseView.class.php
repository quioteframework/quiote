...
class SampleAppBaseView extends View
{
	...
	*/
	...
	// Updated signature to match parent View which now expects ActionInitContext
	#[\Override]
	...
	public final function execute(RequestDataHolder $rd): never
	{
	...
	public function executeHtml(RequestDataHolder $rd): never
	{
		throw new ViewException(sprintf(
			'The View "%1$s" does not implement an "execute%3$s()" method to serve '.
	...
	public function executeJson(RequestDataHolder $rd): never
	{
		throw new ViewException(sprintf(
			'The View "%1$s" does not implement an "execute%3$s()" method to serve '.
			'the Output Type "%2$s". It is recommended that you change the code of '.
